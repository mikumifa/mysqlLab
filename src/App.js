      ...
      <div className="flex min-h-screen flex-col h-full bg-slate-50 text-slate-800 font-sans text-sm select-none" ref={containerRef}>
        ...
        <div className="bg-gradient-to-r from-[#3e0050] to-[#62007e] p-3 flex flex-wrap gap-4 justify-between items-center shadow-md shrink-0 border-b border-purple-900/10 z-50 relative">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-white leading-tight tracking-wide">MySQL 隔离事务 & 锁 LAB</h1>
                    ...
                    <div className="p-4 flex-1 overflow-y-auto space-y-3">
                      <div className="flex justify-between items-center bg-slate-50 p-1.5 rounded border border-slate-200">
                        ...
                        <select
                            ...
                            className="bg-transparent text-xs rounded px-2 py-0.5 text-slate-700 outline-none w-full text-right"
                        >
                        ...
                        <tr key={row.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${isFocused ? 'bg-blue-50/50' : ''}`}>
                          ...
                          <td className="p-2">
                            <div className="flex justify-center flex-wrap gap-1 min-h-[20px]">
                              {rowLocks.map((l, idx) => (
                                  <span key={idx} className={`px-1.5 py-0.5 rounded text-[10px] font-bold border flex items-center ${
                                      l.status === 'WAITING' ? 'bg-yellow-100 text-yellow-700 border-yellow-300 animate-pulse' :
        ...
        {/* 免责声明 */}
        <div className="bg-slate-100 border-t border-slate-200 p-1 text-center text-[10px] text-slate-500 italic select-none">
          免责声明：本系统为前端模拟教学环境，核心逻辑手动实现，具体行为与真实 MySQL 8.0 可能略有出入，仅供参考。
        </div>
      </div>
  );
};
